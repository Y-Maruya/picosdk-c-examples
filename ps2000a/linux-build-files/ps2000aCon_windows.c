			...
			if (fp != NULL)
			{
				...
				for (i = 0; i < sampleCount; i++) 
				{
					...
					for (j = 0; j < unit->channelCount; j++) 
					{
						...
						{
							fprintf(	fp,
								"Ch%C %d %+d %d %+d",//%+d‚ÌŒã‚ë‚Éspace‚ð‘«‚·‚×‚«‚¾‚Á‚½‚ªAˆ—‚ª‚¤‚Ü‚­‚¢‚Á‚Ä‚¢‚é‚Ì‚Å‚»‚Ì‚Ü‚Ü 
								(char)('A' + j),
								buffers[j * 2][i],
...
void CollectRapidBlock(UNIT * unit)
{
	...
	uint32_t nPreTriggerSamples = 100;
	...
	status = ps2000aRunBlock(unit->handle, nPreTriggerSamples, nSamples- nPreTriggerSamples, timebase, 1, &timeIndisposed, 0, CallBackBlock, NULL);
...
int32_t main(void)
{
	...
	while (ch != 'X')
	{
		...
		printf("\n");
		printf("T - Triggered block<-use this                 V - Set voltages\n");
		printf("B - Immediate block                           I - Set timebase\n");
