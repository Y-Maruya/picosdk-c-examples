...
****************************************************************************/
void blockDataHandler(UNIT * unit, char * text, int32_t offset, MODE mode,int datanum, char* BlockFile)
{
	int16_t retry;
	int16_t bit;
	...
	printf("before runblock start Time: %ld.%06ld\n", utcTime.tv_sec, utcTime.tv_usec);
	...
	do
	{
		...
		status = ps3000aRunBlock(unit->handle, sampleCount*0.5, sampleCount*0.5, timebase, oversample, &timeIndisposed, 0, callBackBlock, NULL);
	...
	printf("waiting start Time: %ld.%06ld\n", utcTime.tv_sec, utcTime.tv_usec);
	// printf("Waiting for trigger...Press a key to abort\n");
	...
	{	
		...
		else
		{
			...
			// for (i = sampleCount*0.5-50; i < sampleCount*0.5+50; i++) 
			// {
			...
			if (mode == DIGITAL || mode == MIXED)
			{
				...
				char BlockFiletmp[256];
				strcpy(BlockFiletmp, BlockFile);
				snprintf(futter,sizeof(futter),"/digi_%d.txt", datanum);
				strcat(BlockFiletmp, futter);
				fopen_s(&digiFp, BlockFiletmp, "w");
...
void collectBlockEts(UNIT * unit)
{
	...
	blockDataHandler(unit, "Ten readings after trigger:\n", BUFFER_SIZE / 10 - 5, ANALOGUE,0,"block"); // 10% of data is pre-trigger
...
void collectBlockTriggered(UNIT * unit)
{
	...
	int16_t triggerVoltage = mv_to_adc(-100, unit->channelSettings[PS3000A_CHANNEL_A].range, unit);
...
* Returns       none
***************************************************************************/
void orAnalogueDigitalTriggered(UNIT * unit, int i,char* BlockFile)
{
	int32_t channel = 0;
	...
	digDirections[0].channel = PS3000A_DIGITAL_CHANNEL_0;
	digDirections[0].direction = PS3000A_DIGITAL_DIRECTION_FALLING;
	...
	* Threshold = 1000mV */
	...
	if (setTrigger(unit, NULL, 0, &conditions, 1, &directions, &pulseWidth, 0, 0, 0, digDirections, 1) == PICO_OK)
	{

		blockDataHandler(unit, NULL, 0, MIXED,i,BlockFile);
	}
...
void digitalBlockTriggered(UNIT * unit)
{
	...
	digDirections[1].direction = PS3000A_DIGITAL_DIRECTION_FALLING;

	digDirections[0].channel = PS3000A_DIGITAL_CHANNEL_4;
	digDirections[0].direction = PS3000A_DIGITAL_DONT_CARE;
...
void digitalMenu(UNIT *unit)
{
	...
	while (ch != 'X')
	{
		...
		switch (ch) 
		{
			...
			case 'O':
				orAnalogueDigitalTriggered(unit,0,"block");
				break;
...
int32_t main(void)
{
	...
	while (ch != 'X')
	{
		...
		printf("M - Triggered mix block<-UseThis(triggered by)");
		...
		switch (ch) 
		{
			...
			case 'M':
				...
				for (int i = 0; i < dataOfNum; i++) {
					...
					orAnalogueDigitalTriggered(&unit,i,BlockFile);
					gettimeofday(&tv, NULL);
